...
public sealed class SystemController : ControllerBase
{
    ...
    public ActionResult<SystemStubResult> MonthlyProcess() =>
        Stub("monthly-process", "Monthly process stub â€” not implemented.");

    [HttpPost("day-end")]
    [Authorize(Policy = Policies.RequireAdmin)]
    public ActionResult<SystemStubResult> DayEnd() =>
        Stub("day-end", "Day-end process stub â€” not implemented.");

    [HttpPost("delete-cancelled-document")]
    [Authorize(Policy = Policies.RequireAdmin)]
    public ActionResult<SystemStubResult> DeleteCancelledDocument() =>
        Stub("delete-cancelled-document", "Delete cancelled document stub â€” not implemented.");

    [HttpPost("backup")]
    [Authorize(Policy = Policies.RequireSuperAdmin)]
    public ActionResult<SystemStubResult> Backup() =>
        Stub("backup", "Backup stub â€” not implemented.");

    [HttpPost("restore")]
    [Authorize(Policy = Policies.RequireSuperAdmin)]
    public ActionResult<SystemStubResult> Restore() =>
        Stub("restore", "Restore stub â€” not implemented.");
