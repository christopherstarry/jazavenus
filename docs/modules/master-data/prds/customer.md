...
Customer management with 10 sub-pages for outlet classifications. Customers are the core master data â€” linked to salesmen, collectors, areas, warehouses, payment terms, and price tiers.
...
interface Customer {
  ...
  credit_limit: number;
  salesman_code: string;      // FK â†’ salesmen
  collector_code: string;     // FK â†’ collectors
  distribution_type: string;  // FK â†’ distribution_types
  trade_type: string;         // FK â†’ trade_types
  outlet_type: string;        // FK â†’ outlet_types
  outlet_group_code: string;  // FK â†’ outlet_groups
  outlet_group_type: string;  // FK â†’ outlet_group_types
  payment_term: string;       // FK â†’ payment_terms
  discount_code: string;      // FK â†’ discount_codes
  warehouse_code: string;     // FK â†’ warehouses (default)
  price_tier: string;         // FK â†’ price_tiers
  is_locked: boolean;
  is_active: boolean;
...
// Specific tables:
// Class Outlet â†’ class_parameters (ClassCode, Parameter0-9 for pricing)
// Group Outlet â†’ outlet_groups (GrpOltCode)
// Location Outlet â†’ distribution_types (DstrbnType)
// Market Type â†’ trade_types (TradeType)
// Channel Outlet â†’ sub_trade_types (SubTradeType)
// Outlet Type â†’ outlet_types (OutletType)
// Outlet Group Type â†’ outlet_group_types (GrpOltTypeCode)
```
...
|--------|-----------|------------|----------------------|-----------------|
| View list | âœ… | âœ… | âœ… | âŒ (grayed out) |
| Search/Filter | âœ… | âœ… | âœ… | âŒ |
| Create | âœ… | âœ… | âœ… | âŒ |
| Edit | âœ… | âœ… | âœ… | âŒ |
| Delete | âœ… | âœ… | âŒ (hidden) | âŒ |
| View address sub-tab | âœ… | âœ… | âœ… | âŒ |
| Add/Edit addresses | âœ… | âœ… | âœ… | âŒ |

**Master access users:** Didi, Pai, Nenden, Atep, Alvin â€” can create + edit but NOT delete.
**No Master access:** Yane, Ilham, Robby, Nisa â€” sidebar grayed out.
...
- Click row â†’ detail view with editable fields + address sub-list
...
## 8. History / Audit
Every create, update, and delete on customer records, addresses, salesmen, collectors, areas, and all customer-related lookup tables is logged to `audit_logs`. Developer and SuperAdmin can view the full change history at `/system/audit-history` or click the "Audit History" button on any detail page. See `docs/modules/master-data/prds/audit-history.md` for full spec.
...
- [ ] Lookup tabs (Salesman, Area, etc.) â€” simple CRUD with Code + Name
