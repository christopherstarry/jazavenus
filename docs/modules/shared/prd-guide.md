# PRD Guide for AI â€” How to Write Specs AI Can Execute
...
| Missing data shapes | **Define every field** â€” name, type, required, validation |
...
| Too big (10+ features in one doc) | **One PRD per feature/module** â€” small and scoped |

---

## The PRD Template

Copy this structure for every feature. Fill in ALL sections â€” blank sections make AI guess, and guessing produces bad code.
...
|--------|------|---------|-------------|----------|
| GET | /api/master/class-outlets?search=&page=1&pageSize=20 | List with search & pagination | â€” | `{ items: ClassOutlet[], total: number }` |
...
| DELETE | /api/master/class-outlets/:id | Delete | â€” | `204 No Content` |
...
### Delete
- [ ] Confirmation dialog: "Delete [code] â€” [description]?"
...
1. **Always include Data Model** â€” AI needs to know exact field names and types.
2. **Always define API contracts** â€” even if backend isn't built yet, define the shape.
3. **List all UI states** â€” loading, empty, error, success. Don't make AI guess.
4. **Reference existing files** â€” tell AI what patterns/components to copy.
5. **One PRD per module** â€” don't combine Customer CRUD, Reports, and Auth in one doc.
6. **Use checkboxes** â€” `- [ ]` makes it easy to track what's done.
7. **Be explicit about routes** â€” AI needs exact paths to wire up navigation.
8. **Example data is gold** â€” show one real record so AI understands context.
...
| **UI components** | All in `src/components/ui/` â€” table, dialog, button, input, etc. |
...
Below is a real PRD for the **Purchase Order** screen â€” one of the modules already in the `TREE` but not yet implemented. An AI could build the entire thing from just this doc.
...
- [ ] Click row â†’ navigate to `/purchase/purchase-order/:id`
