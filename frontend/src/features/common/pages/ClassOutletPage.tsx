...
const fields: RefField[] = [
  { key: "name", label: "Name", required: true, placeholder: "Range / description" },
];

export function ClassOutletPage() {
  const { user } = useAuth();
  const canDelete = user?.isDeveloper || user?.roles.includes("SuperAdmin");
  return <ReferenceDataPage title="Class Outlets" apiPath="master/class-outlets" columns={columns} fields={fields} transformDto={(dto) => ({ ...dto, code: genCode() })} canDelete={canDelete} />;
}
