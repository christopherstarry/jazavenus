...
interface PurchaseReturnLineDto {
  id?: string;
  lineNumber: number;
  itemId: string;
  itemSku?: string | null;
...
interface PurchaseReturnDto {
  ...
  goodsReceiptNoteNumber?: string | null;
...
/** Purchase Return — legacy `frmPurchaseReturn` (ObjType 33). See docs/modules/purchase/prds/purchase-return.md. */
export function PurchaseReturnPage() {
  ...
  const [sourceGrn, setSourceGrn] = useState<LookupItem | null>(null);
  ...
  const loadDoc = useCallback((dto: PurchaseReturnDto) => {
    setDocId(dto.id);
    setSupplier({ id: dto.supplierId, code: "", name: dto.supplierName ?? "" });
    setWarehouse({ id: dto.warehouseId, code: dto.warehouseCode ?? "", name: "" });
    setSourceGrn(dto.goodsReceiptNoteId ? { id: dto.goodsReceiptNoteId, code: dto.goodsReceiptNoteNumber ?? "", name: "" } : null);
  ...
  const openDoc = useCallback(async (id: string) => loadDoc(await api.get(`inbound/purchase-returns/${id}`).json<PurchaseReturnDto>()), [loadDoc]);

  const buildPayload = useCallback(
    () => ({
      supplierId: supplier?.id,
      warehouseId: warehouse?.id,
      goodsReceiptNoteId: sourceGrn?.id ?? null,
      returnCode: returnCode?.code ?? null,
      returnDate: returnDate ? new Date(returnDate).toISOString() : null,
      notes: notes || null,
      lines: toSubmitLines(lines).map((l) => ({
        lineNumber: l.lineNumber, itemId: l.itemId, quantity: l.quantity, unitCost: l.unitCost, discountPercent: 0,
      })),
    }),
    [supplier, warehouse, sourceGrn, returnCode, returnDate, notes, lines],
  );
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isNew) loadDoc(await api.post("inbound/purchase-returns", { json: payload }).json<PurchaseReturnDto>());
      else {
        await api.put(`inbound/purchase-returns/${docId}`, { json: payload });
        await openDoc(docId!);
      }
    },
  ...
  const postMutation = useMutation({
    mutationFn: async () => {
      await api.post(`inbound/purchase-returns/${docId}/post`);
      await openDoc(docId!);
    },
  ...
  return (
    <div className="flex flex-col gap-3 p-3 sm:p-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold">{t("purchase:purchaseReturn.title")}</h1>
      ...
      <Card>
        <CardContent className="grid gap-3 pt-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <LookupFieldInput label={t("purchase:purchaseReturn.sourceGrn")} type="grns" code={sourceGrn?.code ?? ""} name={sourceGrn?.name} disabled={readOnly}
            onSelect={(item) => { setSourceGrn(item); markDirty(); }} onClear={() => setSourceGrn(null)} />
          <LookupFieldInput label={t("purchase:purchaseReturn.returnCode")} type="return-codes" code={returnCode?.code ?? ""} name={returnCode?.name} disabled={readOnly}
            onSelect={(item) => { setReturnCode(item); markDirty(); }} onClear={() => setReturnCode(null)} />
          <div className="space-y-1.5">
            <Label className="text-xs uppercase tracking-wide">{t("purchase:purchaseReturn.returnDate")}</Label>
      ...
      <LookupDialog type="purchase-returns" open={browseOpen} onOpenChange={setBrowseOpen} onSelect={(item) => item.id && void openDoc(item.id)} />
      {dialog}
    </div>
  );
}
